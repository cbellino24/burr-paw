(function () {
  ...
  function buildPopup(location, distance) {
    var distanceHtml =
      distance !== null
        ? '<p class="location-popup__distance">' +
          distance.toFixed(1) +
          " mi away</p>"
        : "";
    return (
      '<div class="location-popup">' +
      '<p class="location-popup__category">' +
      ...
      "</p>" +
      distanceHtml +
      '<a class="location-popup__link" href="' +
  ...
  function renderResults(entries) {
    ...
    if (!entries.length) {
      resultsList.innerHTML =
        ...
        "<p>Try a different city or zip, or expand your search radius. More retailers will be added soon.</p>" +
        "</div>";
    ...
    resultsList.innerHTML = entries
      .map(function (entry) {
        ...
        var distanceBadge =
          entry.distance !== null
            ? '<span class="location-card__distance">' +
    ...
    return fetch(url, {
      ...
      .then(function (results) {
        if (!results.length) {
          searchOrigin = null;
          setStatus(
            "We couldn't find that location. Try a nearby city or zip code.",
            true
          );
          refresh();
          return;
      ...
      .catch(function () {
        searchOrigin = null;
        setStatus(
          "Location search is temporarily unavailable. Browse all locations below.",
          true
        );
        refresh();
      });
  ...
  function refresh() {
    ...
    if (
      ...
      !entries.some(function (entry) {
        return entry.location.id === activeId;
      })
    ) {
